...
function POMDPs.transition(mdp::roadnet_with_pursuer,state::roadnet_pursuer_state,action::Symbol)
    # call the generator many times to estimate the probabilities
    # "hack" to make is so we can solve this with ValueIterationSolver as well
    its = 1000
    states = Array{roadnet_pursuer_state}(its)
    rng = MersenneTwister(0)
    for i = 1:its
        states[i] = POMDPs.generate_s(mdp,state,action,rng)
    end

    us = unique(states)
    probs = zeros(length(us))
    dests = Array{roadnet_pursuer_state}(length(probs))
    for (i,state) in enumerate(us)
        probs[i] = count(i->(i==state),states)/its
        dests[i] = state
    end
    #  println("finished")
    return SparseCat(dests,probs)
end
